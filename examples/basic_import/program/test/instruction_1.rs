...
mod instruction_1 {
	...
	fn test_transaction_is_ok() {
		...
		let data = ValidateImportsInstruction::Instruction1;

		let account_info = Keypair::new();
		let validate_accounts_v_0_0_0_info = Keypair::new();
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(account_info.pubkey(), false),
			AccountMeta::new_readonly(validate_accounts_v_0_0_0_info.pubkey(), false),
		];
		...
		create_account(&rpc_client, &fee_payer_info, &account_info, &Pubkey::from_str("8WtjCDLNXNKCDzQHro6vsQT3PTUX4TuLuTbFomMSoMrs").unwrap(), 1);
		create_account(&rpc_client, &fee_payer_info, &validate_accounts_v_0_0_0_info, &program_id, 0);
