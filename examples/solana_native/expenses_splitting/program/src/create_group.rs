...
pub fn create_group(
    ...
    assert!(
        members.len() > 0,
        "members list should provide at least one address"
    );

    assert!(
        initial_balances.len() == members.len(),
        "members and initial balances lists should be of equal size"
    );

    assert!(
        initial_balances.iter().sum::<i64>() == 0,
        "initial balances should sum zero"
    );

    group_account.data.group_name = group_name;
    group_account.data.members = members;
    group_account.data.balances = initial_balances;
