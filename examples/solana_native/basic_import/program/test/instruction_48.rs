...
mod instruction_48 {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("validate_imports", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let data = ValidateImportsInstruction::Instruction48;

		let signer_1_info = Keypair::new();
		let signer_2_info = Keypair::new();
		let signer_3_info = Keypair::new();
		let account_info = Keypair::new();
		let validate_advanced_cases_v_0_0_0_info = Keypair::new();
		let accounts = vec![
			...
			AccountMeta::new(signer_3_info.pubkey(), true),
			AccountMeta::new(account_info.pubkey(), false),
			AccountMeta::new_readonly(system_program::id(), false),
			AccountMeta::new_readonly(validate_advanced_cases_v_0_0_0_info.pubkey(), false),
		];
		...
		create_account(&rpc_client, &fee_payer_info, &signer_1_info, &program_id, 0);
		...
		create_account(&rpc_client, &fee_payer_info, &signer_3_info, &system_program::id(), 0);
		create_account(&rpc_client, &fee_payer_info, &account_info, &program_id, 0);
		create_account(&rpc_client, &fee_payer_info, &validate_advanced_cases_v_0_0_0_info, &program_id, 0);
