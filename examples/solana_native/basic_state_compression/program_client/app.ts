...
async function main(feePayer: Keypair) {
    ...
    await initializeMerkleTreeSendAndConfirm({
        ...
        createMerkleTreeAccountForInitializeMerkleTree: {
            keypair: merkleTree,
            feePayer,
        },
        signers: { feePayer },
    ...
    const treeState: any[] = [];
    try {
        appendChangeLogListener = await onChangelogForMerkleTree(
        ...
        trx = await insertOrAppendArticleSendAndConfirm({
            merkleTree: merkleTree.publicKey,
            root: Array.from(proof.root),
            index: proof.leafIndex,
