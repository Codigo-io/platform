...
mod verify_leaf_article {
	...
	fn test_transaction_is_ok() {
		...
		let leaf: Vec<u8> = Default::default();
		let root: Vec<u8> = Default::default();
		let index: u32 = Default::default();

		let merkle_tree_info = Keypair::new();
		let article_info = Keypair::new();
		let noop_info = Keypair::new();
		let csl_spl_account_compression_v_0_0_0_info = Keypair::new();
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new_readonly(merkle_tree_info.pubkey(), false),
			AccountMeta::new_readonly(article_info.pubkey(), false),
			AccountMeta::new_readonly(noop_info.pubkey(), false),
			AccountMeta::new_readonly(csl_spl_account_compression_v_0_0_0_info.pubkey(), false),
		];
		let data = ValidateStateCompressionInstruction::VerifyLeafArticle(crate::generated::instructions::VerifyLeafArticleArgs{
			leaf,
			root,
			index,
		});
		...
		create_account(&rpc_client, &fee_payer_info, &merkle_tree_info, &Pubkey::from_str("cmtDvXumGCrqC1Age74AVPhSRVXJMd8PJS91L8KbNCK").unwrap(), 35832);
		create_account(&rpc_client, &fee_payer_info, &article_info, &program_id, 2411);
		create_account(&rpc_client, &fee_payer_info, &noop_info, &program_id, 0);
