...
mod initialize_merkle_tree {
	...
	fn test_transaction_is_ok() {
		...
		let data = ValidateStateCompressionInstruction::InitializeMerkleTree;

		let merkle_tree_info = Keypair::new();
		let authority_info = Keypair::new();
		let noop_info = Keypair::new();
		let account_compression_info = Keypair::new();
		...
		let accounts = vec![
			...
			AccountMeta::new_readonly(account_compression_info.pubkey(), false),
		...
		create_account(&rpc_client, &fee_payer_info, &account_compression_info, &program_id, 0);
